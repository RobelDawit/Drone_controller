...
class SupportFilesDrone:
    ...
    def __init__(self):
        ...
        wind_mean= np.array([7.0, 0.0, 0.0])  # Mean wind [m/s]
        sigma_u= 2
        sigma_v = 2
        sigma_w= 2  # Turbulence intensity
        L_u= 30
        L_v = 30
        L_w = 30
        ...
        self.constants={'Ix':Ix, 'Iy':Iy, 'Iz':Iz, 'm':m, 'g':g, 'Jtp':Jtp, 'Ts':Ts,\
            ...
            'rho':rho, 'trajectory':trajectory, 'no_plots':no_plots, 'wind_mean':wind_mean,\
            'sigma_u':sigma_u, 'sigma_v':sigma_v, 'sigma_w':sigma_w,'L_u':L_u,'L_v':L_v,'L_w':L_w}
        
        # Precompute gusts
        t = np.arange(0, 100 + Ts * innerDyn_length, Ts)  # Simulation time
        V = 20.0  # Default airspeed for initialization (m/s)
        try:
            self.gust = np.array([
                self.generate_dryden_gust(t, V, sigma_u, L_u),
                self.generate_dryden_gust(t, V, sigma_v, L_v),
                self.generate_dryden_gust(t, V, sigma_w, L_w)
            ]).T
            print("Gust initialization successful. Shape:", self.gust.shape)
        except Exception as e:
            print("Error initializing gusts:", e)
            self.gust = np.zeros((len(t), 3))  # Fallback to zero gusts
            print("Falling back to zero gusts")

        return None
    
    def generate_dryden_gust(self,t, V, sigma, L):
        # V: Drone airspeed, sigma: Turbulence intensity, L: Scale length, Ts: Sample time
        tau = L / V  # Time constant
        gust = np.zeros(len(t))
        state = 0.0
        for i in range(len(t)):
            noise = np.random.normal(0, sigma * np.sqrt(2 * self.constants['Ts']/ tau))
            state = state * (1 - self.constants['Ts'] / tau) + noise
            gust[i] = state
        return gust
    ...
    def open_loop_new_states(self,states,omega_total,U1,U2,U3,U4,current_time=0):
        ...
        V_d = np.array([u, v, w])  # Drone velocity in body frame
        V_w_inertial = self.constants['wind_mean']  # Mean wind velocity
        V_w = V_w_inertial + self.gust[int(current_time / Ts) % len(self.gust)]
        ...
        V_w_body = np.matmul(R.T, V_w)  # Wind in body frame
        ...
        for j in range(0,4):
            ...
            if drag_switch==1:
                Fd_u=0.5*C_D_u*rho*V_rel[0]**2*A_u
                Fd_v=0.5*C_D_v*rho*V_rel[1]**2*A_v
                Fd_w=0.5*C_D_w*rho*V_rel[2]**2*A_w
            elif drag_switch==0:
                Fd_u=0
                Fd_v=0
                Fd_w=0
